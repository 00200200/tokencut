...
actor MonitorBridge {
    ...
    private func start() throws {
        ...
        let bundled = Bundle.main.object(forInfoDictionaryKey: "UsageTrimExecutable") as? String
        let executable = ProcessInfo.processInfo.environment["USAGETRIM_EXECUTABLE"] ?? bundled ?? "\(home)/.local/bin/usagetrim"
    ...
    func request(_ method: String, paused: Bool? = nil, text: String? = nil,
        ...
        while !buffer.contains(10) {
            ...
            guard !chunk.isEmpty else {
                process = nil
                throw NSError(domain: "UsageTrim", code: 1, userInfo: [NSLocalizedDescriptionKey: "The monitor stopped. Reconnecting on the next refresh."])
            }
            buffer.append(chunk)
            guard buffer.count < 8 * 1024 * 1024 else {
                process?.terminate()
                throw NSError(domain: "UsageTrim", code: 2, userInfo: [NSLocalizedDescriptionKey: "The monitor response is too large."])
            }
        }
        ...
        guard let result = envelope?["result"], (envelope?["id"] as? Int) == sequence else {
            throw NSError(domain: "UsageTrim", code: 3, userInfo: [NSLocalizedDescriptionKey: "The monitor could not complete the request. Check access to local statistics."])
        }
...
@MainActor final class Model: ObservableObject {
    ...
    func export() {
        ...
        panel.nameFieldStringValue = "UsageTrim-statistics.json"
...
struct PanelView: View {
        ...
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("UsageTrim").font(.title3.bold())
            ...
            HStack {
                ...
                Menu {
                    ...
                    Button("Quit UsageTrim") { NSApp.terminate(nil) }.keyboardShortcut("q")
    ...
    @ViewBuilder private func savings(_ data: Snapshot) -> some View {
        Card {
            HStack {
                Text("TODAY · USAGETRIM").font(.caption.bold()).foregroundStyle(.secondary)
                Spacer()
            ...
            if let total = data.today {
                ...
                Text("Results appear after using the UsageTrim wrapper or MCP tools.")
    ...
    @ViewBuilder private func taskMemory(_ memory: TaskMemory?) -> some View {
        ...
        Card {
            ...
            Text("Your client controls compaction. UsageTrim restores notes for the same session after compaction or resume; it does not rewrite chat history.")
                .font(.caption).foregroundStyle(.secondary)
        }
        Card {
