...
def state_dir() -> Path:
    return Path(os.environ.get("USAGETRIM_STATE_DIR", str(Path.home() / ".usagetrim")))
...
def already_wrapped(command: str) -> bool:
    ...
    try:
        ...
        return False
    return any(Path(word).name == "usagetrim" for word in words)
...
def client_name(default: str = "cli") -> str:
    explicit = os.environ.get("USAGETRIM_CLIENT")
