...
from tokencut.core.task_context import TaskContext, identity
...
def context_hook(payload: object, client: str) -> dict:
    ...
    instruction = (
        f"TokenCut task memory: root={json.dumps(root)}, task={json.dumps(task)}. "
        "Use tokencut_context to save a short checkpoint at meaningful milestones or before requesting native compaction; "
        ...
        "If this tool is unavailable, tokencut context accepts the same JSON request on stdin."
    )
    if note["found"]:
...
def install_context_hooks(
    ...
    if not cache_dir.is_absolute():
        raise ValueError("cache_dir must be absolute and match this client's TokenCut MCP cache")
    ...
    command = shlex.join(
        [
            "env",
            f"TOKENCUT_CACHE_DIR={cache_dir}",
            str(executable),
            "context-hook",
            "--client",
            client,
    ...
    if original is not None:
        fd, backup = tempfile.mkstemp(
            prefix=settings.name + ".pre-tokencut-context-", dir=settings.parent
        )
        with os.fdopen(fd, "wb") as stream:
            stream.write(original)
    fd, staging = tempfile.mkstemp(prefix=".tokencut-context-", dir=settings.parent)
    staged = Path(staging)
    try:
