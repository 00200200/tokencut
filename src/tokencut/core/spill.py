...
def spill_threshold_bytes() -> int:
    raw = os.environ.get("TOKENCUT_SPILL_BYTES", str(DEFAULT_SPILL_BYTES))
    try:
        value = int(raw)
    except ValueError:
...
def spill_large_output(
    ...
    *,
    ...
    base = spill_dir
    if base is None:
        cache_dir = Path(os.environ.get("TOKENCUT_CACHE_DIR", str(Path.home() / ".tokencut")))
    ...
    preview = (
        f"[TokenCut spill: {len(raw_bytes):,} bytes → {path}]\n"
        f"[TokenCut: full redacted output: tokencut retrieve {ref_id}]\n"
        f"{body}"
    )
